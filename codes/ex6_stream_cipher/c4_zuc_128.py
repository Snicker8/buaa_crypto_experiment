...
if '__main__' == __name__:
    key_str = input().strip().replace('\r', '').replace('\n', '')[2:]
    iv_str = input().strip().replace('\r', '').replace('\n', '')[2:]
    n = int(input())
    key = []
    iv = []
    for i in range(16):
        key.append(int(key_str[2 * i: 2 * (i + 1)], 16))
        iv.append(int(iv_str[2 * i: 2 * (i + 1)], 16))
    zuc = ZUC(key, iv)
    # 加密过程
    Ks = zuc.generate_keystream(n)
    for K in Ks:
        print('0x' + '%08x' % K)
