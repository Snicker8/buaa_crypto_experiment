...
uint64_t ede(uint64_t input,uint64_t key1,uint64_t key2)
{
    uint64_t result;
    result = des(input,key1,'e');
    result = des(result,key2,'d');
...
int main(int argc, const char * argv[]) {
    ...
    {
        scanf("0x%016llx\n",&DT);
        R=ede(V^ede(DT,key1,key2),key1,key2);
        V=ede(R^ede(DT,key1,key2),key1,key2);
        printf("0x%016llx\n",R);
    }
    scanf("0x%016llx",&DT);
    R=ede(V^ede(DT,key1,key2),key1,key2);
