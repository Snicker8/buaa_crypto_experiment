def h2b(x):
    return bin(int(x, 16))[2:].zfill(4 * len(x))


def b2h(x):
    return hex(int(x, 2))[2:].zfill(len(x) // 4)


def xor(x, y):
    return bin(int(x, 2) ^ int(y, 2))[2:].zfill(len(x))
...
def S(R):
    ...
    for i in range(8):
        row = int(Rs[i][0] + Rs[i][5], 2)
        col = int(Rs[i][1: 5], 2)
        Rs[i] = bin(s[i][row][col])[2:].zfill(4)
    R = ''
