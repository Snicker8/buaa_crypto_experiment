...
def xor(a, b, n):
    return hex(int(a, 16) ^ int(b, 16))[2:].zfill(n)
...
def OFB(s, k, iv, n):
    ...
    i = 0
    o = Encrypt(iv, k)
    c_temp = xor(s[0:2 * n], o[0:2 * n], 2 * n)
    c += c_temp
    s_len -= 2 * n
    i += 1
    while s_len >= 2 * n:
        iv = iv[2 * n:] + c_temp
        o = Encrypt(iv, k)
        c_temp = xor(s[2 * n * i:2 * n * (i + 1)], o[0:2 * n], 2 * n)
        c += c_temp
        i += 1
        s_len -= 2 * n
    # 尾部
    if s_len > 0:
        iv = iv[2 * n:] + c_temp
        o = Encrypt(iv, k)
        c += xor(s[2 * n * i:], o[0:s_len], s_len)
    return c


def OFB_inv(s, k, iv, n):
    s_len = len(s)
    c = ''
    # 第0轮
    i = 0
